...
class MainViewModel : ViewModel() {
    ...
    val hokkaidoState = _prefectureInArea.distinctUntilChangedBy { it[Prefecture.Hokkaido.ordinal] }.map {
        it[Prefecture.Hokkaido.ordinal]
    }.stateIn(viewModelScope, SharingStarted.Eagerly, PrefectureState.None(Prefecture.Hokkaido))

    val okinawaState = _prefectureInArea.distinctUntilChangedBy { it[Prefecture.Okinawa.ordinal] }.map {
